    ...
    <div className="min-h-[80vh] grid place-items-center px-4">
      <form onSubmit={onSubmit} className="w-full max-w-sm rounded-[24px] bg-white border border-[#F1D9D0] p-8 card-shadow">
        ...
        <div className="mt-6 rounded-xl bg-[#FFF8F0] border border-[#F1D9D0] p-3 text-[11px] leading-relaxed text-[#3D4A5E]">
          <strong>Demo credentials (change via .env):</strong><br/>
          ID: <code>admin</code> / <code>tripnaari</code><br/>
          Password: <code>TripNaari2026!</code> or <code>change_me_secure</code> (from ADMIN_PASSWORD)<br/><br/>
          Set <code>ADMIN_PASSWORD</code> in .env for production. For Vercel, add env variable.
        </div>
      </form>
    </div>
  );
